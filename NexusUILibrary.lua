...
║  Fixed:   UIBlur crash, window not showing, content layout, shadow,         ║
║           ClipsDescendants clipping popups, self._conn typo,                ║
║           ContextMenu animation, Button:setDisabled, syncShadow timing      ║
╚══════════════════════════════════════════════════════════════════════════════╝
]]
...
function ModalSystem:show(opts)
	...
	overlay.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			if not dialog:IsAncestorOf(input.Position and game) then
				closeModal()
			end
		end
	end)
...
function ContextMenu:show(items, position)
	...
	-- Fade in (no Size tween — conflicts with AutomaticSize)
	menu.BackgroundTransparency = 1
	Utils.tween(menu, T.AnimSpeed, { BackgroundTransparency = 0 })
...
function LoadingSpinner.new(parent, theme, opts)
	...
	elseif style == "pulse" then
		...
		local function pulsate()
			if not holder.Parent then return end
			Utils.tween(circle, 0.5, { Size = UDim2.fromScale(1.2, 1.2), BackgroundTransparency = 0.6 }, Enum.EasingStyle.Sine)
			task.delay(0.5, function()
				if not holder.Parent then return end
				Utils.tween(circle, 0.5, { Size = UDim2.fromScale(0.8, 0.8), BackgroundTransparency = 0 }, Enum.EasingStyle.Sine)
...
function UIWindow.new(screenGui, theme, opts)
	...
	localwindow = Utils.newInstance("Frame", {
		Name            = "Window",
		Size            = UDim2.fromOffset(width, height * 0.9),
	...
	task.delay(T.AnimSpeed * 1.5, syncShadow)
...
function NexusUI.new(opts)
	...
	gui.DisplayOrder   = 10
	gui.Parent         = opts.Parent or playerGui or game:GetService("CoreGui")
